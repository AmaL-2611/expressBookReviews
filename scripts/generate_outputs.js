...
async function run() {
  try {
    // 1 githubrepo - we cannot query GitHub here; provide curl command placeholder
    await save('githubrepo', "curl -i https://api.github.com/repos/<your-username>/<your-repo>", { note: 'Run this locally after forking from ibm-developer-skills-network/expressBookReview' });

    // 2 getallbooks
    const all = (await axios.get(`${BASE}/books`)).data;
    await save('getallbooks', `curl -s ${BASE}/books`, all);

    // 3 getbooksbyISBN
    const isbn = Object.keys(all)[0];
    const byIsbn = (await axios.get(`${BASE}/books/isbn/${isbn}`)).data;
    await save('getbooksbyISBN', `curl -s ${BASE}/books/isbn/${isbn}`, byIsbn);

    // 4 getbooksbyauthor
    const author = all[isbn].author;
    const byAuthor = (await axios.get(`${BASE}/books/author/${encodeURIComponent(author)}`)).data;
    await save('getbooksbyauthor', `curl -s ${BASE}/books/author/${encodeURIComponent(author)}`, byAuthor);

    // 5 getbooksbytitle
    const title = all[isbn].title;
    const byTitle = (await axios.get(`${BASE}/books/title/${encodeURIComponent(title)}`)).data;
    await save('getbooksbytitle', `curl -s ${BASE}/books/title/${encodeURIComponent(title)}`, byTitle);

    // 6 getbookreview
    const review = (await axios.get(`${BASE}/books/${isbn}/review`)).data;
    await save('getbookreview', `curl -s ${BASE}/books/${isbn}/review`, review);
